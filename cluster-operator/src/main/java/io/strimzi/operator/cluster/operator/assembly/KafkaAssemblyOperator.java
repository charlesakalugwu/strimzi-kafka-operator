...
public class KafkaAssemblyOperator extends AbstractAssemblyOperator<KubernetesClient, KafkaAssembly, KafkaAssemblyList, DoneableKafkaAssembly, Resource<KafkaAssembly, DoneableKafkaAssembly>> {
    ...
    private final Future<Void> createOrUpdateKafka(Reconciliation reconciliation, KafkaAssembly assemblyCm, List<Secret> assemblySecrets) {
        ...
        getKafkaClusterDescription(assemblyCm, assemblySecrets)
                //.compose(desc -> desc.withVoid(cro.reconcile(KafkaCluster.getInitContainerClusterRoleName(desc.kafka().getName()), desc.kafka.generateClusterRole())))
                .compose(desc -> desc.withVoid(getReconcile(namespace, desc)))
                //.compose(desc -> desc.withVoid(crbo.reconcile(KafkaCluster.getInitContainerClusterRoleBindingName(desc.kafka().getName()), desc.kafka.generateClusterRoleBinding(namespace))))
    ...
    private Future<ReconcileResult<ServiceAccount>> getReconcile(String namespace, KafkaClusterDescription desc) {
        return serviceAccountOperator.reconcile(namespace, KafkaCluster.getInitContainerServiceAccountName(desc.kafka().getName()), desc.kafka.generateInitContainerServiceAccount());
    }
